...
const UpdateInterviewForm = () => {
  ...
  async function interviewScheduleInfo() {
    try {
      ...
      setInterviewInfo(Array.isArray(data) ? data : [data]); // Ensure data is in array format
    } catch (error) {
  ...
  const handleChange = (e) => {
    ...
    // Check if uploaded file is an image and its size is within the limit
    if (
      (files && !["image/jpeg", "image/png", "image/jpg"].includes(files[0].type)) ||
      (files && files[0].size > 20 * 1024 * 1024) // 20 MB limit
    ) {
      setNotificationMessage("Invalid file format or size. Please upload a JPG, JPEG, or PNG image within 20 MB.");
  ...
  useEffect(() => {
    // Reset selected option to default when component mounts or form is refreshed
    setSelectedOption(defaultOption);
  }, [defaultOption]);

  const handleDropdownChange = (e) => {
    setSelectedOption(e.target.value);
    setShowInput(e.target.value !== ""); // Toggle input field based on dropdown value
  };
  ...
  const handleEditClick = (item) => {
    setSelectedRow(item); // Set the selected row data for editing
